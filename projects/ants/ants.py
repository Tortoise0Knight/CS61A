...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if not self.is_container and place.ant.is_container and place.ant.can_contain(self):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount < self.health:
            ...
            reflective_damage = amount + self.damage
...
# BEGIN Problem 7
class HungryAnt(Ant):
    """HungryAnt eat a bee from its place and chews for 3 turns"""
...
class TankAnt(ContainerAnt):
    """TankAnt proctects while attacking"""
...
# END Problem 12
    ...
    def inspire_single(self, target):
        if target not in self.inspired_ants:
            target.double()
            self.inspired_ants.append(target)
        ...
        while place:
            if place.ant:
                if place.ant.is_container and place.ant.ant_contained:
                    self.inspire_single(place.ant.ant_contained)
                self.inspire_single(place.ant)
            place = place.exit
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not place.is_hive:
            ...
            if place.ant:
                if place.ant.is_container and place.ant.ant_contained:
                    insects_dict[place.ant.ant_contained] = distance
                insects_dict[place.ant] = distance
            place = place.entrance
            distance += 1
        del insects_dict[self]
        return insects_dict
        # END Problem Optional 2
