...
def boar_brawl(player_score, opponent_score):
    ...
    gain = 3 * abs(player_ones - opponent_tens)
    return max(1, gain)
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score in range(goal):
        for opponent_score in range(goal):
            num_rolls_list.append(strategy(score, opponent_score))
    return all(element == num_rolls_list[0] for element in num_rolls_list)
    # END PROBLEM 7
