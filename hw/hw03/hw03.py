...
def interleaved_sum(n, odd_term, even_term):
    ...
    """
    "*** YOUR CODE HERE ***"
    def interleaved_helper(i, this_term):
        if i == n:
            return this_term(i)
        else:
            if this_term == odd_term:
                next_term = even_term
            else:
                next_term = odd_term
            return this_term(i) + interleaved_helper(i+1, next_term)
    return interleaved_helper(1, odd_term)
...
def count_coins(total):
    ...
    def count(sum, coin):
        if sum == 0:
            ...
            return count(sum, next_smaller_coin(coin)) + count(sum-coin, coin)
